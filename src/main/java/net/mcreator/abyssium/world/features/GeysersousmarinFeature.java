...
public class GeysersousmarinFeature extends Feature<NoneFeatureConfiguration> {
	public static final GeysersousmarinFeature FEATURE = (GeysersousmarinFeature) new GeysersousmarinFeature()
			.setRegistryName("abyssium2:geysersousmarin");
	...
	private final List<Block> base_blocks = List.of(Blocks.SAND, Blocks.GRAVEL);
	private StructureTemplate template = null;

	public GeysersousmarinFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("abyssium2", "geyser"));
		if (template == null)
			return false;
